...
class SkinDAO {
    ...
    function save($package) {
        ...
        if (!FileSystem::make_dir($this->source . $name)) {
            // The target directory could not be created
            return false;
        }
        
        return $this->unzip($tmpfile, $this->source . $name);
    }
    ...
    function delete($name) {
        if ($this->exists($name)) {
            return FileSystem::delete_dir("{$this->source}{$name}/", false);
        }
        return false;
    }
    
    function index() {
        $data = FileSystem::list_dirs($this->source, 0);
        $this->data = array();
    ...
    function exists($name) {
        return (!empty($name) && file_exists($this->source . $name));
    }
